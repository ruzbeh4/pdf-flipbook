...
var currentPdf = 'https://ruzbeh4.github.io/pdf-flipbook/Portfolio.pdf'; // Default PDF URL

function applyDefaultFullscreen(book) {
    if (!book || !book.container) {
        return;
    }
    ...
    if (typeof container.hasClass === 'function' && !container.hasClass('df-fullscreen')) {
        container.addClass('df-fullscreen');
    }
...
function loadFlipbook(pdfUrl, rtlMode, page, pdfId) {
    ...
    var options = {
        ...
        pageMode: (window.DFLIP && DFLIP.PAGE_MODE
            ? DFLIP.PAGE_MODE.DOUBLE
            : 2),
        singlePageMode: (window.DFLIP && DFLIP.SINGLE_PAGE_MODE
        ...
        onReady: function(book) {
            ...
            // Mirror the fullscreen toggle so the viewer occupies the full viewport on load
            setTimeout(function () {
                applyDefaultFullscreen(book);
            }, 0);
        },
    };
